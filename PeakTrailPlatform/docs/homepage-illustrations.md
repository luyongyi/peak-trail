...
## 手机与代码审查入口（2026-09-24）

- 首页新增公开仓库、当前 DLL 对应源码提交、版本 Release 链接；链接与发行 manifest 通过测试核对。
- 320/390 px 手机竖屏保留完整四关、默认终章插画、安装教程和可展开配置；短横屏也独立适配。
- 回放的地图与玩家面板纵向排列，时间轴固定底部，增加「玩家 / 地图」跳转。触屏自由相机提供六向按住移动按钮。
- 在桌面浏览器尺寸模拟中检查手机、iPad 和桌面，验证合成多人数据的状态卡、拖动时间与倍速；不等同真机 Safari/Android 测试。
...
真实分支决定终章：火山 → 熔炉，雾沼 → 城塞。四关下方默认展开本轮终章插画，
...
- 当时的设备样式回归仅覆盖 iPad/桌面；现已根据新增需求恢复手机断点，并更新回归测试。
